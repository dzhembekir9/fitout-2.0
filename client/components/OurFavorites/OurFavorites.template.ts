import { OurFavoritesProps } from './OurFavorites.props'

export const ourFavoritesData: OurFavoritesProps = {
  tabs: [
    {
      title: 'FOR EVERYDAY',
      cards: [
        {
          title: 'Tree Runner',
          label: 'Light and Breezy Sneaker1',
          image: {
            url: 'https://i.imgur.com/rGcPLMc.png',
            width: 450,
            height: 450,
          },
          labelIcon: 'https://i.imgur.com/g5xa8LI.png',
        },
        {
          title: 'Tree Piper',
          label: 'Classic Low Top Sneaker',
          image: {
            url: 'https://i.imgur.com/INA8rCO.png',
            width: 450,
            height: 450,
          },
          labelIcon: 'https://i.imgur.com/g5xa8LI.png',
        },
        {
          title: 'Tree Lounger',
          label: 'Breezy Slip-On',
          image: {
            url: 'https://i.imgur.com/43LYVg2.png',
            width: 450,
            height: 450,
          },
          labelIcon: 'https://i.imgur.com/g5xa8LI.png',
        },
      ],
    },
    {
      title: 'FOR RUNNING',
      cards: [
        {
          title: 'Tree Dasher 2',
          label: 'For short-medium distance runs',
          image: {
            url: 'https://i.imgur.com/Ia1oKvH.png',
            width: 450,
            height: 450,
          },
          labelIcon: 'https://i.imgur.com/g5xa8LI.png',
        },
        {
          title: 'Trail Runner SWT',
          label: 'For trail runs',
          image: {
            url: 'https://i.imgur.com/TM8S8iF.png',
            width: 450,
            height: 450,
          },
          labelIcon: 'https://i.imgur.com/BuZVB3h.png',
        },
        {
          title: 'Natural Run Long Sleeve Tee',
          label: 'Ultra-light and temperature-regulating',
          image: {
            url: 'https://i.imgur.com/DCbKS0s.png',
            width: 450,
            height: 450,
          },
          labelIcon: 'https://i.imgur.com/g5xa8LI.png',
        },
      ],
    },
    {
      title: 'FOR LOUNGING',
      cards: [
        {
          title: 'Sea Long Sleeve Tee',
          label: 'Soft And Fresh Basics',
          image: {
            url: 'https://i.imgur.com/rq2GbQC.png',
            width: 450,
            height: 450,
          },
          labelIcon: 'https://i.imgur.com/87MiVCH.png',
        },
        {
          title: 'Wool Lounger',
          label: 'Cozy Slip-On',
          image: {
            url: 'https://i.imgur.com/1zQoHPE.png',
            width: 450,
            height: 450,
          },
          labelIcon: 'https://i.imgur.com/VZbGqP8.png',
        },
        {
          title: 'R&R Collection',
          label: 'Soft-To-The-Touch Sweats',
          image: {
            url: 'https://i.imgur.com/I5m4DEI.png',
            width: 450,
            height: 450,
          },
          labelIcon: 'https://i.imgur.com/VZbGqP8.png',
        },
      ],
    },
  ],
}
